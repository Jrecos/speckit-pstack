...
Prefer the Slack tool the user configured. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it, for a narrow missing capability such as editing this one status message, and read it from the host's secret store. Never expose the token to a worker.
