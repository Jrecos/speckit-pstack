...
![She tastes a finished dish while robots cook from a recipe box, with pinned cards reading /speckit.pstack.how, /speckit.pstack.tdd, and the run instruction above the counter.](./images/recipes.jpg)

## Understand an unfamiliar subsystem

```text
use /speckit.pstack.how first to understand how this initialization works. then use /speckit.pstack.why to figure out why it broke recently.
```
...
```text
ask /speckit.pstack.arena for a second opinion on this thread and our approach
```
...
```text
/speckit.pstack.interrogate the whole branch, but skeptically. don't change anything yet. no nitpicks unless it's an actual bug or regression in behavior.
```
...
```text
/speckit.pstack.poteto-mode repro the duplicate write first. if there's a cheap test path, /speckit.pstack.tdd it. then fix and rerun.
```
...
```text
/speckit.pstack.unslop that, no emdashes
```
...
That's the whole prompt. [`/speckit.pstack.bro`](../../skills/bro/SKILL.md) restates the last message like one human talking to another, no jargon, shorter. Use it when a reply is technically thorough and you still don't know what it said.

## The pitfalls

- **Enumerating skills in the prompt.** "use /speckit.pstack.how then /speckit.pstack.architect then /speckit.pstack.arena" reorders steps the playbook already sequences. State the goal and constraints. Name a skill only to override a default.
- **A vague finish condition.** "make it better" gives a long run nothing to check. Give a command or artifact that can pass or fail.
...
- **Using `/speckit.pstack.arena` for coverage.** `/speckit.pstack.arena` repeats one design or code brief, then picks a base and grafts the best parts. `/speckit.pstack.swarm` partitions slices or declared race arms and aggregates one report.
- **Accepting every review comment.** Bots and humans both file real catches and noise in one list. `/speckit.pstack.interrogate` sorts findings into act-on and dismissed buckets with reasons, and you can override either way.
