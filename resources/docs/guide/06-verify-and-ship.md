...
```text
/speckit.pstack.poteto-mode add json output to this command. text output stays byte-identical, the json parses, both run against the sample project. show me the evidence.
```
...
For a small diff you don't fully trust, [`/speckit.pstack.blast-radius`](../../skills/blast-radius/SKILL.md) finds what it could break elsewhere. It picks the one fact the change is safe because of and proves it by running code instead of writing an essay about it.
...
```text
/speckit.pstack.poteto-mode babysit this pr. get it green.
```
...
```text
/speckit.pstack.poteto-mode check on pr 123. anything outstanding?
```
...
```text
/speckit.pstack.poteto-mode land the stack.
```
