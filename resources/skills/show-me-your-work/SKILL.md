...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for /speckit.pstack.show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away."
...
Use the helper `.specify/extensions/pstack/resources/skills/show-me-your-work/scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>`. It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from this workspace's transcript directory (`.specify/extensions/pstack/runtime/pstack-native.py capability-report` says whether the host exposes it; `PSTACK_TRANSCRIPTS_DIR` carries the path). Don't search another project's store: that reads unrelated private chats. Walk the log against what actually happened:
