...
Before spawning investigators, list the MCP servers this session actually has. Use the host's available-tools listing when present. Otherwise read the host's MCP configuration. A source you cannot reach is reported as unavailable, not assumed.
...
- `model`: role `why investigators` from the project role map (`.specify/extensions/pstack/runtime/pstack-native.py role-plan --role "why investigators"`); omit the model when the leg inherits the current one
...
- subagent: the host's general-purpose subagent
- `model`: role `why synthesizer` from the project role map (`.specify/extensions/pstack/runtime/pstack-native.py role-plan --role "why synthesizer"`); omit the model when the leg inherits the current one
...
Take the synthesizer's output and present it to the user. For workflow dispatch, read the complete answer from the validated report's non-empty `result`, not its evidence excerpt. You may lightly edit for clarity or add context from the conversation, but **do not rewrite the confidence language**.
