---
name: architect
description: "Sketch types, signatures, and module structure before code, then stay in the loop while implementation fills in. Use for /speckit.pstack.architect, 'architect this', 'design this', or non-trivial work where jumping to code would lock in the wrong shape."
...
Resolve role `architect runners` from the project role map (`.specify/extensions/pstack/runtime/pstack-native.py role-plan --role "architect runners"`), one runner per configured entry, duplicates preserved. An empty role is a setup error, not a reason to pick a model yourself.
...
Opt in to a checkpoint when the invoker explicitly asks: "/speckit.pstack.architect with checkpoint," "stop and show me before implementing," or similar. Then surface the synthesized design and pause for sign-off.
