...
5. Loop, one hypothesis per iteration:
   - Hand the change to a subagent on role `hillclimb` from the project role map (`.specify/extensions/pstack/runtime/pstack-native.py role-plan --role "hillclimb"`); omit the model when the leg inherits the current one with a tight scope. Supervise and review the diff rather than typing it (the **guard-the-context-window** principle skill). When several independent hypotheses are live, fan them to parallel subagents, each in its own worktree (the **separate-before-serializing-shared-state** principle skill).
