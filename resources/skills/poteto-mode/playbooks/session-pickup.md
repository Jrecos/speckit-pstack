...
1. Locate the prior trail. A transcript from this workspace's transcript directory (`.specify/extensions/pstack/runtime/pstack-native.py capability-report` says whether the host exposes it; `PSTACK_TRANSCRIPTS_DIR` carries the path. Do not search another project's store, that crosses workspace boundaries and reads private chats from unrelated projects), a pushed branch, or a handed-off brief from another agent. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
