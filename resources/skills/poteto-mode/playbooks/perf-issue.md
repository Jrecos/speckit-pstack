...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent on role `perf-issue` from the project role map (`.specify/extensions/pstack/runtime/pstack-native.py role-plan --role "perf-issue"`); omit the model when the leg inherits the current one. Review the diff. Capture a post-fix trace.
