...
**PRs.** Run the packaged deslop reference (`.specify/extensions/pstack/resources/dependencies/deslop.md`) over the diff before commit. Run `/speckit.pstack.no-comments` before review. Write every PR title, PR description, and commit body with `/speckit.pstack.technical-writing`, then apply `/speckit.pstack.unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
...
A subagent that opens a PR runs the packaged `interrogate` skill, the deslop reference at `.specify/extensions/pstack/resources/dependencies/deslop.md`, and `/speckit.pstack.no-comments`. It returns the URL and does not babysit. Return to the parent.
